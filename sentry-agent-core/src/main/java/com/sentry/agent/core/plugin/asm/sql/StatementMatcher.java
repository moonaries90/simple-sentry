...
public class StatementMatcher implements Matcher<AsmMethod> {
    ...
    public boolean match(AsmMethod target) {
        if((ACC_PUBLIC & target.access) != 0) {
            ...
            return argTypes.length > 0
                    && argTypes[0].equals(Type.getType(String.class))
                    && returnType.equals(methodReturnTypes.get(target.name));
        }
        return false;
    }
